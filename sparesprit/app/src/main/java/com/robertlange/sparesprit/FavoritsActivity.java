...
public class FavoritsActivity extends Activity implements
    ...
    private class FuelServiceAccessTask extends AsyncTask<String, Void, String> {
        ...
        private static final String DataByIdsUri =
                "http://www.sparesprit.de/api/v2/FuelApi.php?request=databyids";

        @Override
        protected void onPreExecute() {
            if(!FavoritsActivity.this.isWorking) {
                linlaHeaderProgress.setVisibility(View.VISIBLE);
                FavoritsActivity.this.isWorking = true;
            }
        }

        @Override
        protected String doInBackground(String... urls) {
            List<FuelStation> fs = new ArrayList<FuelStation>();
        ...
        protected void onPostExecute(String result) {
            ...
            FavoritsActivity.this.isWorking = false;
        ...
        private List<FuelStation> GetDataByIds(android.location.Location loc, String article, float distance, String sortBy) throws RESTException, JSONException {

            Set<Integer> favorits = FavoritsActivity.this.readPrefs("favorits");
            JSONArray mJSONArray = new JSONArray(new ArrayList<Integer>(favorits));

            String homeApiRequestString = DataByIdsUri + "&latitude=" + loc.getLatitude() + "&longitude=" + loc.getLongitude()
                    + "&article=" + article + "&distance=" + distance + "&sortBy=" + sortBy + "&ids=" + mJSONArray.toString() ;
